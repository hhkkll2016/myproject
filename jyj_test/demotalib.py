...
#测试数据，后续通过自动筛选得来
##df_list = ['002281','000938','600487','002444','002296','600662','600547']
df_list = ["002281","000938","600487","002444","002296","600662","600547"]
...
##        df = ts.get_k_data(code,ktype='D',autype='qfq',index='False',start='',end='')
...
#在后面增加3列，分别是8-10列,13-15列，对应的是 DIFF  DEA  DIFF-DEA       
            ...
            if df.iat[(dflen-1),7]>0:
                if df.iat[(dflen-1),8]>0:
                    if df.iat[(dflen-1),7]>df.iat[(dflen-1),8]:
            ...
            if df.iat[(dflen-1),9]>0 and dflen >30 :
                for i in range(1,26):
                    if df.iat[(dflen-1-i),9]<=0:#
                        operate = operate + 1
                        break
            #由正变负，卖出信号   
            if df.iat[(dflen-1),9]<0 and dflen >30 :
...
'''       
##df = Get_Stock_List()
df2 = Get_MACD(df_list)
##Dist = 'C:\\Python36\\jyj_test\\'
##Output_Csv(df,Dist)
