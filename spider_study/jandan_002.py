...
class Spider(threading.Thread):
    ...
    def get_Page(self, page_Url, proxies, urlMannager):
        ...
        constant = re.search(r'.*f_\w+\(e,\"(\w+)\".*', jsFile)
        print(constant)
...
def main(amount, firstIndex):
    ...
    for i in range(amount):
        page_Url.append('http://jandan.net/ooxx/page-' + str(firstIndex - i) + '#comments')
    page_threads = []
    t_amount = 10 if len(page_Url) > 10 else len(page_Url)  # 页面抓取线程数
    for i in range(t_amount):
        t = Spider(page_Url[int((len(page_Url)) / t_amount) * i:int((len(page_Url)) / t_amount) * (i + 1)], proxies,
                   urlMannager)
